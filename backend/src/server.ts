...
// CORS configuration. Accepts:
//   - exact origin matches from CORS_ORIGIN (comma-separated)
//   - any *.vercel.app origin if VERCEL_ALLOW_PREVIEW=true (handy for PR previews)
//   - a wildcard "*" entry (development only)
// Denied origins are logged so misconfiguration is obvious in Vercel logs.
const corsAllowlist = process.env.CORS_ORIGIN!.split(',').map((s) => s.trim()).filter(Boolean);
const allowVercelPreviews = process.env.VERCEL_ALLOW_PREVIEW === 'true';
const allowAny = corsAllowlist.includes('*');
...
app.use(
  cors({
    origin: (origin, cb) => {
      ...
      // eslint-disable-next-line no-console
      console.warn(`[cors] denied origin "${origin}" — add it to CORS_ORIGIN env var (comma-separated)`);
...
app.get('/api/admissions', authenticateToken, async (req: any, res: Response) => {
  try {
    ...
    // Admission has no direct tenantId — scope through Patient.tenantId so
    // tenants can never see each other's admissions.
    const where: any = { patient: { tenantId: req.user.tenantId } };
...
app.get('/api/hr/employees', authenticateToken, async (req: any, res: Response) => {
  try {
    ...
    // Surface the meta in the body in dev/preview so we can diagnose
    // schema-drift errors (P2022 etc) directly from a curl. Stripped in
    // production deploys to avoid leaking internals to end-users.
    const isProd = process.env.NODE_ENV === 'production' && process.env.VERCEL_ENV === 'production';
    res.status(500).json({
      error: 'Internal server error',
      code: error?.code,
      ...(isProd ? {} : { meta: error?.meta, message: error?.message }),
    });
  }
});
...
// All asset endpoints scope by req.user.tenantId so one tenant can never see
// another's equipment. assetCode is also unique-per-tenant (see schema).
...
app.post('/api/assets', authenticateToken, async (req: any, res: Response) => {
  try {
    ...
    // Auto-generate asset code (unique per tenant) if not provided
    let code: string = (assetCode || '').trim();
    if (!code) {
      const last = await prisma.asset.findFirst({
        where: { tenantId: req.user.tenantId, assetCode: { startsWith: 'AST' } },
    ...
    if (error?.code === 'P2002') {
      return res.status(409).json({ error: 'assetCode already exists for this tenant' });
    }
...
app.get('/api/assets/:id/maintenance', authenticateToken, async (req: any, res: Response) => {
  try {
    const owns = await prisma.asset.findFirst({ where: { id: req.params.id, tenantId: req.user.tenantId } });
    if (!owns) return res.status(404).json({ error: 'Asset not found' });
    const logs = await prisma.maintenanceLog.findMany({
      where: { assetId: req.params.id, tenantId: req.user.tenantId },
